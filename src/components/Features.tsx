...
export default function Features() {
  return (
    <section id="features" className="relative py-28 px-6 bg-black/20">
      <div className="container max-w-7xl mx-auto">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <div className="text-xs uppercase tracking-[0.3em] text-accent/80 mb-4 font-mono font-semibold">
            Built for UVCE
          </div>
          <h2 className="font-display text-4xl md:text-6xl font-bold tracking-tight text-ink-50">
            Everything you need to <span className="gradient-text italic font-bold">crack</span> the paper.
          </h2>
          <p className="mt-5 text-ink-200/70 text-sm">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {FEATURES.map((f, i) => (
            ...
            >
              <div
                className={`absolute -top-12 -right-12 h-32 w-32 rounded-full bg-gradient-to-br ${f.accent} opacity-20 blur-2xl group-hover:opacity-40 transition duration-500`}
              />
              <div
              ...
              <h3 className="mt-5 font-display text-xl font-bold text-ink-50">{f.title}</h3>
              <p className="mt-2 text-sm text-ink-200/70 leading-relaxed font-mono text-[13px]">{f.body}</p>
            </motion.div>
          ))}
        </div>
      </div>
